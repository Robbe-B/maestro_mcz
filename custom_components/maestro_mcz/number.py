        ...
        if supported_numbers is not None:
            for supported_number in supported_numbers:
                if supported_number[0] is not None and supported_number[1] is not None:
                    entities.append(
                        MczNumberEntity(stove, supported_number[0], supported_number[1])
                    )
...
class MczNumberEntity(CoordinatorEntity, NumberEntity):
    ...
    def __init__(
        ...
        if (
            matching_number_configuration.configuration.type == TypeEnum.INT.value
            or matching_number_configuration.configuration.type == TypeEnum.DOUBLE.value
        ):
    ...
    def handle_coordinator_update_internal(self) -> None:
        if hasattr(self.coordinator._maestroapi.Status, self._prop):
            self._attr_native_value = getattr(
                self.coordinator._maestroapi.Status, self._prop
            )
        elif hasattr(self.coordinator._maestroapi.State, self._prop):
            self._attr_native_value = getattr(
                self.coordinator._maestroapi.State, self._prop
            )
        else:
